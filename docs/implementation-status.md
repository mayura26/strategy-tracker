...
- Combo workbench overlap analytics with all-win, mixed-day, correlation, component contribution table, and a mode-routing lab for signal-based one-mode-per-day routing with Mode B or cash fallback.
...
- Visual comparison workspace with same-mode scoped run selection, overlap-only or union date handling, selectable full-run vs selected-period core metric bars, outperformance-vs counts with material-delta filtering, CSV exports for visible/material day lists, filtered daily PnL overlays, green/red day summaries, daily PnL histograms, and daily/session box and dot plots.
- Mode routing rules under `/combos` for same-bot cross-mode routing, using prior-session RSI, prior ATR, opening-range percentage, and previous 15:45-16:00 range percentage threshold rules to simulate one-mode-per-day routed PnL with non-selected modes treated as zero exposure, including Mode B or cash fallback.
- Saved routing rules for preserving promising Mode A/Mode B routing rules with evaluation snapshots.
